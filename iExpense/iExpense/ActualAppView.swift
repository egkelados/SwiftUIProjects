...
//    @State private var user = UserDefaults.standard
    ...
    var filteredPersonalExpenses: [ExpenseItem] {
        expenses.items.filter { $0.type == "Personal" && (searchText.isEmpty || $0.name.localizedCaseInsensitiveContains(searchText)) }
    }

    var filteredBusinessExpenses: [ExpenseItem] {
        expenses.items.filter { $0.type == "Buiseness" && (searchText.isEmpty || $0.name.localizedCaseInsensitiveContains(searchText)) }
    }

    // calculating the total amount into the view
        ...
        NavigationStack {
            List {
                Section("Total expenses") {
                    Text(totalExpenses, format: .currency(code: Locale.current.currency?.identifier ?? "USD"))
                }

                Section("Personal Expenses") {
                    ForEach(filteredPersonalExpenses, id: \.id) { item in
                    ...
                    .onDelete(perform: removeItemsFromList)
                }

                Section("Buiseness Expenses") {
                    ForEach(filteredBusinessExpenses, id: \.id) { item in
